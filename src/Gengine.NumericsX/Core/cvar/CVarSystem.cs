...
namespace Gengine.NumericsX.Core
{
    ...
    internal class CVarSystemLocal : ICVarSystem
    {
        ...
        public bool Command(CmdArgs args)
        {
            ...
            {
                // print the variable
                common.Printf($"\"{local.nameString}\" is:\"{local.valueString}\"{S_COLOR_WHITE} default:\"{local.resetString}\"\n");
                if (local.Description.Length > 0)
                    common.Printf($"{S_COLOR_WHITE}{local.Description}\n");
            }
            // set the value
        ...
        static void Toggle_f(CmdArgs args)
        {
            ...
            if (cvar == null)
            {
                common.Warning($"Toggle_f: cvar \"{args[1]}\" not found");
                return;
            }
            ...
            if (argc > 3)
            {
                ...
                common.Printf($"set {args[1]} = {args[i]}\n");
            ...
            else
            {
                ...
                common.Printf($"set {args[1]} = {current}\n");
                ...
                case SHOW.VALUE:
                    {
                        foreach (var cvar in cvarList)
                            common.Printf($"{cvar.nameString:-32}{S_COLOR_WHITE} \"{cvar.valueString}\"\n");
                        break;
                    }
                case SHOW.DESCRIPTION:
                    {
                        ...
                        foreach (var cvar in cvarList)
                            common.Printf($"{cvar.nameString:-32}{S_COLOR_WHITE}{CreateColumn(cvar.Description, NUM_DESCRIPTION_CHARS, indent, b)}\n");
                        break;
                    }
                case SHOW.TYPE:
                    {
                        foreach (var cvar in cvarList)
                        {
                            if ((cvar.Flags & CVAR.BOOL) != 0)
                                common.Printf($"{cvar.Name:-32}{S_COLOR_CYAN}bool\n");
                            else if ((cvar.Flags & CVAR.INTEGER) != 0)
                            {
                                if (cvar.MinValue < cvar.MaxValue) common.Printf($"{cvar.Name:-32}{S_COLOR_GREEN}int {S_COLOR_WHITE}[{(int)cvar.MinValue}, {(int)cvar.MaxValue}]\n");
                                else common.Printf($"{cvar.Name:-32}{S_COLOR_GREEN}int\n");
                            }
                            else if ((cvar.Flags & CVAR.FLOAT) != 0)
                            {
                                if (cvar.MinValue < cvar.MaxValue) common.Printf($"{cvar.Name:-32}{S_COLOR_RED}float {S_COLOR_WHITE}[{cvar.MinValue}, {cvar.MaxValue}]\n");
                                else common.Printf($"{cvar.Name:-32}{S_COLOR_RED}float\n");
                            }
                            else if (cvar.ValueStrings != null)
                            {
                                common.Printf($"{cvar.Name:-32}{S_COLOR_WHITE}string {S_COLOR_WHITE}[");
                                for (var j = 0; cvar.ValueStrings[j] != null; j++)
                                    if (j != 0) common.Printf($"{S_COLOR_WHITE}, {cvar.ValueStrings[j]}");
                                    else common.Printf($"{S_COLOR_WHITE}{cvar.ValueStrings[j]}");
                                common.Printf($"{S_COLOR_WHITE}]\n");
                            }
                            else common.Printf($"{cvar.Name:-32}{S_COLOR_WHITE}string\n");
                        }
                        break;
                    }
                case SHOW.FLAGS:
