...
namespace Gengine.UI
{
    public class UserInterfaceLocal : IUserInterface
    {
        ...
        public void ReadFromDemoFile(VFileDemo f)
        {
            ...
            if (add)
                uiManagerLocal.demoGuis.Add(this);
        }
        ...
        public void RecurseSetKeyBindingNames(Window window)
        {
            ...
            if (v != null)
                SetStateString(v.Name, KeyInput.KeysFromBinding(v.Name));
            var i = 0;
    ...
    public class UserInterfaceManagerLocal : IUserInterfaceManager
    {
        ...
        public void BeginLevelLoad()
        {
            var c = guis.Count;
            for (var i = 0; i < c; i++)
                if ((guis[i].Desktop.Flags & Window.WIN_MENUGUI) == 0)
                    guis[i].ClearRefs();
        }
        ...
        public IUserInterface FindGui(string qpath, bool autoLoad = false, bool needInteractive = false, bool forceNotUnique = false)
        {
            ...
            for (var i = 0; i < c; i++)
                ...
                {
                    if (!forceNotUnique && (needInteractive || guis[i].IsInteractive))
                        break;
                    guis[i].AddRef();
                    return guis[i];
                }
        ...
        public IUserInterface FindDemoGui(string qpath)
        {
            ...
            for (var i = 0; i < c; i++)
                if (string.Equals(demoGuis[i].SourceFile, qpath, StringComparison.OrdinalIgnoreCase))
                    return demoGuis[i];
            return null;
        }
