...
namespace System.NumericsX.OpenStack.Render
{
    #region Records
    ...
    {
        static void VERBOSE(string x) { if (ase.verbose) Printf(x); }
        ...
        static void ASE_ParseBracedBlock(Action<string> parser)
        {
            ...
            while (ASE_GetToken(false))
                if (ase.token == "{") indent++;
                else if (ase.token == "}") { --indent; if (indent == 0) break; else if (indent < 0) Error("Unexpected '}'"); }
        ...
        static void ASE_KeyMESH_VERTEX_LIST(string token)
        {
            ...
            if (token == "*MESH_VERTEX")
            {
                ...
                ase.currentVertex++;
                if (ase.currentVertex > pMesh.numVertexes) Error("ase.currentVertex >= pMesh.numVertexes");
            }
            else Error($"Unknown token '{token}' while parsing MESH_VERTEX_LIST");
        }
        ...
        static void ASE_KeyMESH_FACE_LIST(string token)
        {
            ...
            if (token == "*MESH_FACE")
            {
                ...
                //    mtlID = int.Parse(p);
                //}
                //else Error("No *MESH_MTLID found for face!");

                ase.currentFace++;
            }
            else Error($"Unknown token '{token}' while parsing MESH_FACE_LIST");
        }
        ...
        static void ASE_KeyTFACE_LIST(string token)
        {
            ...
            else Error($"Unknown token '{token}' in MESH_TFACE");
        }
        ...
        static void ASE_KeyCFACE_LIST(string token)
        {
            ...
            else Error($"Unknown token '{token}' in MESH_CFACE");
        }
        ...
        static void ASE_KeyMESH_TVERTLIST(string token)
        {
            ...
            if (token == "*MESH_TVERT")
            {
                ...
                if (ase.currentVertex > pMesh.numTVertexes) Error("ase.currentVertex > pMesh.numTVertexes");
            }
            else Error($"Unknown token '{token}' while parsing MESH_TVERTLIST");
        }
        ...
        static void ASE_KeyMESH_CVERTLIST(string token)
        {
            ...
            if (token == "*MESH_VERTCOL")
            {
                ...
                if (ase.currentVertex > pMesh.numCVertexes) Error("ase.currentVertex > pMesh.numCVertexes");
            }
            else Error($"Unknown token '{token}' while parsing MESH_CVERTLIST");
        }
        ...
        static void ASE_KeyMESH_NORMALS(string token)
        {
            ...
            if (token == "*MESH_FACENORMAL")
            {
                ...
                if (num >= pMesh.numFaces || num < 0) Error($"MESH_NORMALS face index out of range: {num}");
                if (num != ase.currentFace) Error("MESH_NORMALS face index != currentFace");
            ...
            else if (token == "*MESH_VERTEXNORMAL")
            {
                ...
                if (num >= pMesh.numVertexes || num < 0) Error($"MESH_NORMALS vertex index out of range: {num}");
                ...
                if (v == 3) Error("MESH_NORMALS vertex index doesn't match face");
        ...
        static void ASE_KeyMESH(string token)
        {
            ...
            else if (token == "*MESH_TFACELIST") { if (pMesh.faces == null) Error("*MESH_TFACELIST before *MESH_FACE_LIST"); ase.currentFace = 0; VERBOSE(".....parsing MESH_TFACE_LIST\n"); ASE_ParseBracedBlock(ASE_KeyTFACE_LIST); }
            else if (token == "*MESH_CFACELIST") { if (pMesh.faces == null) Error("*MESH_CFACELIST before *MESH_FACE_LIST"); ase.currentFace = 0; VERBOSE(".....parsing MESH_CFACE_LIST\n"); ASE_ParseBracedBlock(ASE_KeyCFACE_LIST); }
            else if (token == "*MESH_NORMALS") { if (pMesh.faces == null) Warning("*MESH_NORMALS before *MESH_FACE_LIST"); ase.currentFace = 0; VERBOSE(".....parsing MESH_NORMALS\n"); ASE_ParseBracedBlock(ASE_KeyMESH_NORMALS); }
        }
        ...
        static void ASE_KeyMESH_ANIMATION(string token)
        {
            ...
            else Error($"Unknown token '{token}' while parsing MESH_ANIMATION");
        }
        ...
        public static AseModel ASE_Parse(byte[] buffer, bool verbose)
        {
            ...
            while (ASE_GetToken(false))
            {
                ...
                else if (!string.IsNullOrEmpty(ase.token)) { Printf($"Unknown token '{ase.token}'\n"); }
            }
