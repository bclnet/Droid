...
namespace Droid.Sys
{
    public static partial class SysW
    {
        ...
        static K rightAltKey = K_ALT;

        public static void InitScanTable()
        {
            var lang = Lib.cvarSystem.GetCVarString("sys_lang");
            if (lang.Length == 0)
                lang = "english";
            if (string.Equals(lang, "english", System.StringComparison.OrdinalIgnoreCase))
            {
                keyScanTable = s_scantokey;
                // the only reason that english right alt binds as K_ALT is so that users who were using right-alt before the patch don't suddenly find
                // that only left-alt is working.
                rightAltKey = K_ALT;
            }
            else if (string.Equals(lang, "spanish", System.StringComparison.OrdinalIgnoreCase))
            {
                keyScanTable = s_scantokey_spanish;
                rightAltKey = K_RIGHT_ALT;
            }
            else if (string.Equals(lang, "french", System.StringComparison.OrdinalIgnoreCase))
            {
                keyScanTable = s_scantokey_french;
                rightAltKey = K_RIGHT_ALT;
            }
            else if (string.Equals(lang, "german", System.StringComparison.OrdinalIgnoreCase))
            {
                keyScanTable = s_scantokey_german;
                rightAltKey = K_RIGHT_ALT;
            }
            else if (string.Equals(lang, "italian", System.StringComparison.OrdinalIgnoreCase))
            {
        ...
        public static K Win_MapKey(int key)
        {
            ...
            if (is_extended)
                switch (result)
                {
                    ...
                    case (K)0x0D: return K_KP_ENTER;
                    case (K)0x2F: return K_KP_SLASH;
                    case (K)0xAF: return K_KP_PLUS;
