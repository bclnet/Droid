...
namespace Droid.Core
{
    ...
    public class Lexer
    {
        ...
        public bool LoadMemory(string ptr, int length, string name, int startLine = 1)
        {
            if (loaded)
            {
        ...
        public bool ReadToken(out Token token)
        {
            ...
            else if ((flags & LEXFL.ALLOWPATHNAMES) != 0 && (c == '/' || c == '\\' || c == '.'))
            {
                if (!ReadName(token))
                    return false;
            }
        ...
        public bool ExpectTokenType(TT type, int subtype, out Token token)
        {
            ...
            else if (token.type == TT.PUNCTUATION)
            {
                ...
                {
                    Error($"expected '{GetPunctuationFromId(subtype)}' but found '{token}'");
                    return false;
                }
            }
            return true;
        ...
        public int ParseInt()
        {
            ...
            {
                ExpectTokenType(TT.NUMBER, Token.TT_INTEGER, out token);
                return -token.IntValue;
            }
            else if (token.type != TT.NUMBER || token.subtype == Token.TT_FLOAT)
