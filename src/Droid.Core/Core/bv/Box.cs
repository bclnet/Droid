...
namespace Droid.Core
{
    public class Box
    {
        ...
        public Box() { }
        public Box(Vector3 center, Vector3 extents, Matrix3x3 axis)
        {
            this.center = center;
        ...
        public Box(Bounds bounds, Vector3 origin, Matrix3x3 axis)
        {
            this.center = (bounds[0] + bounds[1]) * 0.5f;
            this.extents = bounds[1] - this.center;
        ...
        public static Box operator +(Box _, Vector3 t)               // returns translated box
            => new(_.center + t, _.extents, _.axis);
        public static Box operator *(Box _, Matrix3x3 r)             // returns rotated box
            => new(_.center * r, _.extents, _.axis * r);
        public static Box operator +(Box _, Box a)
        {
            Box newBox = new(_);
            newBox.AddBox(a);
            return newBox;
        ...
        public bool AddBox(Box a)                      // add the box, returns true if the box expanded
        {
            if (a.extents.x < 0f)
                return false;
        ...
        public Box Translate(Vector3 translation)  // return translated box
            => new(center + translation, extents, axis);
        public Box TranslateSelf(Vector3 translation)      // translate this box
        {
            center += translation;
            return this;
        }
        public Box Rotate(Matrix3x3 rotation)          // return rotated box
            => new(center * rotation, extents, axis * rotation);
        public Box RotateSelf(Matrix3x3 rotation)          // rotate this box
        {
            center *= rotation;
            axis *= rotation;
            return this;
        ...
        {
            var lp = p - center;
            if (MathX.Fabs(lp * axis[0]) > extents.x ||
        ...
        public bool IntersectsBox(Box a)           // includes touching
        {
        ...
        public void FromBoxTranslation(Box box, Vector3 translation)
            => throw new NotImplementedException();
        ...
        public void FromBoxRotation(Box box, Rotation rotation)
            => throw new NotImplementedException();
        ...
        public void AxisProjection(Matrix3x3 ax, out Bounds bounds)
        {
            bounds = new Bounds();
            for (var i = 0; i < 3; i++)
            {
        ...
        public int GetProjectionSilhouetteVerts(Vector3 projectionOrigin, Vector3[] silVerts)
        {
        ...
        public int GetParallelProjectionSilhouetteVerts(Vector3 projectionDir, Vector3[] silVerts)
        {
            ToPoints(out var points);
