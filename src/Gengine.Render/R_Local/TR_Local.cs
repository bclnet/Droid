...
namespace Gengine.Render
{
    ...
    partial class TR
    {
        public static ScreenRect R_ScreenRectFromViewFrustumBounds(Bounds bounds)
            => new()
            {
                x1 = (short)MathX.FtoiFast(0.5f * (1.0f - bounds[1].y) * (tr.viewDef.viewport.x2 - tr.viewDef.viewport.x1)),
                x2 = (short)MathX.FtoiFast(0.5f * (1.0f - bounds[0].y) * (tr.viewDef.viewport.x2 - tr.viewDef.viewport.x1)),
                y1 = (short)MathX.FtoiFast(0.5f * (1.0f + bounds[0].z) * (tr.viewDef.viewport.y2 - tr.viewDef.viewport.y1)),
                y2 = (short)MathX.FtoiFast(0.5f * (1.0f + bounds[1].z) * (tr.viewDef.viewport.y2 - tr.viewDef.viewport.y1))
            };
	...
	zNear = 8;
            ...
            ymax = (float)(zNear * Math.Tan(tr.viewDef.renderView.fov_y * MathX.PI / 360.0f));
            ymin = -ymax;

            xmax = (float)(zNear * Math.Tan(tr.viewDef.renderView.fov_x * MathX.PI / 360.0f));
            xmin = -xmax;
	...
	tr.viewDef.projectionMatrix[14] = -2.0f*zFar*zNear/(zFar-zNear);
        ...
        static void R_ConstrainViewFrustum_()
        {
            ...
            if (r_useFrustumFarDistance.Float > 0.0f) tr.viewDef.viewFrustum.MoveFarDistance(r_useFrustumFarDistance.Float);
        }
        ...
        public static void R_TransformEyeZToWin(float src_z, float[] projectionMatrix, out float dst_z)
        {
            ...
            if (clip_w <= 0.0f) dst_z = 0.0f; // clamp to near plane
