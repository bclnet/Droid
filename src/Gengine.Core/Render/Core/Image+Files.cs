...
namespace Gengine.Render
{
    static unsafe partial class R
    {
        ...
        static void LoadJPG(string filename, ref byte* pic, out int width, out int height, out DateTime timestamp)
        {
            ...
            out_ = (byte*)R_StaticAlloc(cinfo.output_width * cinfo.output_height * 4);
        ...
        static void R_LoadImage(string cname, ref byte* pic, out int width, out int height, out DateTime timestamp, bool makePowerOf2)
        {
            var name = cname;
        ...
        internal static bool R_LoadCubeImages(string imgName, Image.CF extensions, byte** pics, out int outSize, out DateTime timestamp)
        {
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                if (pics != byteX.empty && extensions == Image.CF.CAMERA)
                    switch (i)
