...
namespace Gengine.Render
{
    static unsafe partial class R
    {
        ...
        static bool R_ParseImageProgram_r(Lexer src, ref byte* pic, out int width, out int height, ref DateTime timestamps, ref Image.TD depth)
        {
            ...
            if (string.Equals(token, "heightmap", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // process it
                if (pic != byteX.empty) { R_HeightmapToNormalMap(pic, width, height, scale); depth = Image.TD.BUMP; }
                MatchAndAppendToken(src, ")");
                return true;
            }
            ...
            if (string.Equals(token, "addnormals", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // process it
                if (pic != byteX.empty) { R_AddNormalMaps(pic, width, height, pic2, width2, height2); R_StaticFree(pic2); depth = Image.TD.BUMP; }
                MatchAndAppendToken(src, ")");
                return true;
            }
            ...
            if (string.Equals(token, "smoothnormals", StringComparison.OrdinalIgnoreCase))
            {
                MatchAndAppendToken(src, "(");
                if (!R_ParseImageProgram_r(src, ref pic, out width, out height, ref timestamps, ref depth)) return false;
                if (pic != byteX.empty) { R_SmoothNormalMap(pic, width, height); depth = Image.TD.BUMP; }
                MatchAndAppendToken(src, ")");
                return true;
            }
