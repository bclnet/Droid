...
namespace Gengine.Render
{
    static partial class R
    {
        ...
        static void LoadBMP(string name, out byte[] pic, out int width, out int height, out DateTime timestamp)
        {
            ...
            if (pic==null) { fileSystem.ReadFile(name, null, timestamp); return; } // just getting timestamp
            ...
            bmpHeader.fileSize = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.reserved0 = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.bitmapDataOffset = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.bitmapHeaderSize = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.width = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.height = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.planes = LittleShort(*(short*)buf_p); buf_p += 2;
            bmpHeader.bitsPerPixel = LittleShort(*(short*)buf_p); buf_p += 2;
            bmpHeader.compression = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.bitmapDataSize = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.hRes = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.vRes = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.colors = LittleInt(*(int*)buf_p); buf_p += 4;
            bmpHeader.importantColors = LittleInt(*(int*)buf_p); buf_p += 4;
        ...
        static void LoadPCX(string filename, out byte[] pic, out byte[] palette, out int width, out int height, out DateTime timestamp)
        {
            byte* raw;
            Pcx* pcx;
            int x, y;
            ...
            o = (byte*)R_StaticAlloc((ymax + 1) * (xmax + 1));
        ...
        static void R_LoadImage(string cname, out byte[] pic, out int width, out int height, out DateTime timestamp, bool makePowerOf2)
        {
            ...
            if ((width && *width < 1) || (height && *height < 1))
                if (pic && *pic) { R_StaticFree(*pic); *pic = 0; }
            ...
            if (pic && *pic && makePowerOf2)
            {
                ...
                {
                    if (globalImages.image_roundDown.Bool && scaled_width > w) scaled_width >>= 1;
                    if (globalImages.image_roundDown.Bool && scaled_height > h) scaled_height >>= 1;
        ...
        static bool R_LoadCubeImages(string imgName, CubeFiles extensions, byte[] pics, out int outSize, out DateTime timestamp)
        {
            ...
            if (i != 6)
            {
                ...
                if (pics != null) for (j = 0; j < i; j++) R_StaticFree(pics[j]);
                timestamp = 0;
                return false;
            }
