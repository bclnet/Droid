    ...
    public class EventLoop
    {
        ...
        void PushEvent(SysEvent ev)
        {
            ...
            if (com_pushedEventsHead - com_pushedEventsTail >= MAX_PUSHED_EVENTS)
            {
                ...
                if (!PushEvent_printedWarning)
                {
                    PushEvent_printedWarning = true;
                    common.Printf("WARNING: Com_PushEvent overflow\n");
                }

                if (pushedEvent.evPtr != IntPtr.Zero)
                    Marshal.FreeHGlobal(pushedEvent.evPtr);
        ...
        void ProcessEvent(SysEvent ev)
        {
            ...
            // free any block data
            if (ev.evPtr != IntPtr.Zero)
                Marshal.FreeHGlobal(ev.evPtr);
        }
