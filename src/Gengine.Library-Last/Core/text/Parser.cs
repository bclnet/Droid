...
using static Gengine.Library.Core.Token;
using static Gengine.Library.Core.Lexer;
using System;
...
namespace Gengine.Library.Core
{
    ...
    public class Parser
    {
        ...
        public bool LoadFile(string filename, bool osPath = false)
        {
            ...
            if (loaded)
            {
                common.FatalError("Parser::loadFile: another source already loaded");
                return false;
            }
        ...
        public bool LoadMemory(string ptr, int length, string name)
        {
            ...
            if (loaded)
            {
                common.FatalError("Parser::loadMemory: another source already loaded");
                return false;
            }
        ...
        bool ReadSourceToken(out Token token)
        {
            ...
            if (scriptstack == null)
            {
                common.FatalError("Parser::ReadSourceToken: not loaded");
                token = null;
                return false;
            }
            changedScript = 0;
        ...
        {
            common.Printf($"define.name = {define.name}\n");
            common.Printf($"define.flags = {define.flags}\n");
            common.Printf($"define.builtin = {define.builtin}\n");
            common.Printf($"define.numparms = {define.numparms}\n");
        }
