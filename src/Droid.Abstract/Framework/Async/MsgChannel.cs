    ...
    public class MsgChannel
    {
        ...
        public bool Process(Netadr from, int time, BitMsg msg, out int sequence)
        {
            ...
            if (sequence <= incomingSequence)
            {
                if (net_channelShowDrop.Bool || net_channelShowPackets.Bool)
                    common.Printf($"{SysW.NetAdrToString(remoteAddress)}: out of order packet {sequence} at {incomingSequence}\n");
                return false;
            }
            ...
            if (fragmented)
            {
                ...
                {
                    if (net_channelShowDrop.Bool || net_channelShowPackets.Bool)
                        common.Printf($"{SysW.NetAdrToString(remoteAddress)}: dropped a message fragment at seq {sequence}\n");
                ...
                {
                    if (net_channelShowDrop.Bool || net_channelShowPackets.Bool)
                        common.Printf($"{SysW.NetAdrToString(remoteAddress)}: illegal fragment length\n");
                    UpdatePacketLoss(time, 0, 1);
                    return false;
                }
        ...
        bool ReadMessageData(BitMsg o, BitMsg msg)
        {
            ...
            while (reliableMessageSize != 0)
            {
                ...
                {
                    common.Printf($"{SysW.NetAdrToString(remoteAddress)}: bad reliable message\n");
                    return false;
                }
