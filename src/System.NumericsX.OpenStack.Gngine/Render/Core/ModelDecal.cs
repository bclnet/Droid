    ...
    public unsafe class RenderModelDecal
    {
        ...
        public static bool CreateProjectionInfo(out DecalProjectionInfo info, in FixedWinding winding, in Vector3 projectionOrigin, bool parallel, float fadeDepth, Material material, int startTime)
        {
            ...
            if (parallel)
                ...
                for (var i = 0; i < winding.NumPoints; i++)
                    info.boundingPlanes[i].FromPoints(projectionOrigin, winding[i].ToVec3(), winding[(i + 1) % winding.NumPoints].ToVec3());
            ...
            d0 = b.ToVec3() - a.ToVec3(); d0.s = b.s - a.s; d0.t = b.t - a.t;
            d1 = c.ToVec3() - a.ToVec3(); d1.s = c.s - a.s; d1.t = c.t - a.t;
        ...
        public static RenderModelDecal RemoveFadedDecals(RenderModelDecal decals, int time)
        {
            ...
            for (i = 0; i < decals.tri.numIndexes; i++) decals.tri.indexes[i] = inUse[decals.tri.indexes[i]];
        ...
        public void AddDecalDrawSurf(ViewEntity space)
        {
            ...
            var newTri = R_FrameAlloc<SrfTriangles>();
            newTri = tri;

            // copy the current vertexes to temp vertex cache
            newTri.ambientCache = vertexCache.AllocFrameTemp(tri.verts, tri.numVerts * sizeof(DrawVert), false);
            newTri.indexCache = vertexCache.AllocFrameTemp(tri.indexes, tri.numIndexes * sizeof(GlIndex), true);
        ...
        void AddDepthFadedWinding(in Winding w, Material decalMaterial, Plane[] fadePlanes, float fadeDepth, int startTime)
        {
            FixedWinding front, back;
