...
namespace Droid.Framework
{
    partial class SessionLocal
    {
        ...
        public override void ReadCDKey()
        {
            ...
            if (!f)
            {
                G.common.Printf($"Couldn't read {filename}.\n");
                cdkey[0] = '\0';
            }
            else
            {
        ...
        public override void WriteCDKey()
        {
            ...
            OSPath = fileSystem.BuildOSPath(G.cvarSystem.GetCVarString("fs_configpath"), Config.BASE_GAMEDIR, Config.CDKEY_FILE);
            fileSystem.CreateOSPath(OSPath);
            f = fileSystem.OpenFileWrite(filename, "fs_configpath");
            if (!f)
            {
                G.common.Printf($"Couldn't write {filename}.\n");
                return;
            }
        ...
        public override string GetCDKey(bool xp)
        {
            if (!xp)
                return cdkey;
            if (xpkey_state == CDKEY.OK || xpkey_state == CDKEY.CHECKING)
                return xpkey;
            return null;
        }
        /// <summary>
        ...
        public override void CDKeysAuthReply(bool valid, string auth_msg)
        {
            ...
            if (!valid)
            {
                G.common.DPrintf("auth key is invalid\n");
                authMsg = auth_msg;
            ...
            {
                G.common.DPrintf("client is authed in\n");
                if (cdkey_state == CDKEY.CHECKING)
                    cdkey_state = CDKEY.OK;
                if (xpkey_state == CDKEY.CHECKING)
                    xpkey_state = CDKEY.OK;
        ...
        public bool MaybeWaitOnCDKey()
        {
            ...
            {
                authWaitBox = true;
                G.sessLocal.MessageBox(MSG.WAIT, G.common.GetLanguageDict().GetString("#str_07191"), null, true, null, null, true);
                return true;
            }
            return false;
        }
        ...
        void EmitGameAuth()
        {
            ...
            if (AsyncNetwork.client.SendAuthCheck(cdkey_state == CDKEY.CHECKING ? cdkey : null, xpkey_state == CDKEY.CHECKING ? xpkey : null))
            {
                ...
                G.common.DPrintf("authing with the master..\n");
            }
            else
            {
                // net is not available
                G.common.DPrintf("sendAuthCheck failed\n");
                if (cdkey_state == CDKEY.CHECKING)
                    cdkey_state = CDKEY.OK;
                if (xpkey_state == CDKEY.CHECKING)
                    xpkey_state = CDKEY.OK;
