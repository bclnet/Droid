    ...
    public class ListWindow : Window
    {
        ...
        public override string HandleEvent(SysEvent ev, Action<bool> updateVisuals)
        {
            ...
            if (currentSel.Count > 0)
                for (var i = 0; i < currentSel.Count; i++)
                    gui.SetStateInt($"{listName}_sel_{i}", currentSel[i]);
            else
                gui.SetStateInt($"{listName}_sel_0", 0);
            ...
            for (var i = top; i < count; i++)
            {
                ...
                if (tabInfo.Count > 0)
                {
                    ...
                    while (start < listItems[i].Length)
                    {
                        ...
                        if (tabInfo[tab].type == TAB_TYPE.TEXT)
