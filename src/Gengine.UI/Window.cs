...
namespace Gengine.UI
{
    ...
    public class Window
    {
        ...
        public void SetupFromState()
        {
            ...
            CalcClientRect(0, 0);
            if (scripts[(int)SCRIPT.ON_ACTION] != null)
            {
                cursor = DeviceContext.CURSOR.HAND;
                flags |= WIN_CANFOCUS;
            }
        }
        ...
        public void SetupBackground()
        {
            if (backGroundName.Length != 0)
            {
                ...
                if (background != null && !background.TestMaterialFlag(MF.DEFAULTED))
        ...
        public int GetWinVarOffset(WinVar wv, DrawWin owner)
        {
            ...
            {
                ret = drawWindows[i].win != null ? drawWindows[i].win.GetWinVarOffset(wv, owner)
        ...
        public virtual bool Parse(Parser src, bool rebuild = true)
        {
            ...
            while (token != "}")
            {
                ...
                else if (token == "onTime")
                {
                    ...
                    if (!ParseScript(src, ev.event_, ev.time))
                    {
                        ret = false;
                        break;
                    }
        ...
        {
            if (R.r_skipGuiShaders.Integer == 1 || dc == null)
                return;

            var time = gui.Time;

            if ((flags & WIN_DESKTOP) != 0 && R.r_skipGuiShaders.Integer != 3)
                RunTimeEvents(time);

            if (R.r_skipGuiShaders.Integer == 2)
                return;
            ...
            if (!cvarSystem.GetCVarBool("draw_pda")) // only do the following if we aren't drawing pda
                if ((flags & WIN_DESKTOP) != 0) // DG: allow scaling menus to 4:3
                    ...
                    if ((flags & WIN_SCALETO43) != 0 || ((flags & WIN_MENUGUI) != 0 && R.r_scaleMenusTo43.Bool))
                    {
                        fixupFor43 = true;
            ...
            {
                if (fixupFor43)
                    dc.SetMenuScaleFix(false); // DG: gotta reset that before returning this function
                return;
            }
...
for (i = 0; i < c; i++)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        public void ReadSaveGameString(out string s, VFile savefile)
        {
            ...
            if (len < 0)
                common.Warning("Window::ReadSaveGameString: invalid length");
        ...
        public virtual void ReadFromSaveGame(VFile savefile)
        {
            ...
            savefile.Read(out byte cursor); this.cursor = (DeviceContext.CURSOR)cursor;
            ...
            savefile.Read(out byte textAlign); this.textAlign = (DeviceContext.ALIGN)textAlign;
        ...
        public void FixupParms()
        {
            ...
            for (i = 0; i < c; i++)
                timeLineEvents[i].event_.FixupParms(this);
        ...
        {
            if (!force && test >= 0 && test < MAX_EXPRESSION_REGISTERS && EvalRegs_lastEval == this)
                return EvalRegs_regs[test];
        ...
        public int ExpressionConstant(float f)
        {
            ...
            for (i = (int)WEXP_REG.NUM_PREDEFINED; i < expressionRegisters.Count; i++)
                if (!registerIsTemporary[i] && expressionRegisters[i] == f)
                    return i;
            if (expressionRegisters.Count == MAX_EXPRESSION_REGISTERS)
            {
                common.Warning($"expressionConstant: gui {gui.SourceFile} hit MAX_EXPRESSION_REGISTERS");
                return 0;
            }
            ...
            if (i > c)
                while (i > c)
                {
                    expressionRegisters.Add(-9999999);
                    i--;
                }
        ...
        {
            for (var find = 0; find < drawWindows.Count; find++)
        ...
        public void RemoveChild(Window win)
        {
            ...
            for (find = 0; find < drawWindows.Count; find++)
                if (drawWindows[find].win == win)
                {
                    drawWindows.RemoveAt(find);
                    break;
                }
        }
        ...
        public bool UpdateFromDictionary(Dictionary<string, string> dict)
        {
            ...
            foreach (var kv in dict)
            {
                ...
                if (!ParseInternalVar(kv.Key, src) && !ParseRegEntry(kv.Key, src))
                    continue;
            }
        ...
        protected int ExpressionTemporary()
        {
            if (expressionRegisters.Count == MAX_EXPRESSION_REGISTERS)
            {
                common.Warning($"expressionTemporary: gui {gui.SourceFile} hit MAX_EXPRESSION_REGISTERS");
                return 0;
            }
        ...
        {
            if (ops.Count == MAX_EXPRESSION_OPS)
            {
                common.Warning($"expressionOp: gui {gui.SourceFile} hit MAX_EXPRESSION_OPS");
                return ops[0];
            }
            WexpOp wop = new();
        ...
        protected object ParseTerm(Parser src, WinVar var = null, object component = null)
        {
            ...
            if (string.Equals(token, "time", StringComparison.OrdinalIgnoreCase))
                return (int)WEXP_REG.TIME;

            // parse negative numbers
            if (token == "-")
            {
                src.ReadToken(out token);
                if (token.type == TT.NUMBER || token == ".")
                    return ExpressionConstant(-(float)token.FloatValue);
                src.Warning($"Bad negative number '{token}'");
                return 0;
            }

            if (token.type == TT.NUMBER || token == "." || token == "-")
                return ExpressionConstant((float)token.FloatValue);
        ...
        protected void EvaluateRegisters(float[] registers)
        {
            ...
            var oc = ops.Count;
            // copy the constants
            for (i = (int)WEXP_REG.NUM_PREDEFINED; i < erc; i++)
