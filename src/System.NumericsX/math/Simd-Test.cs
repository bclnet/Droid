namespace System.NumericsX
{
    public partial class SIMD
    {
        ...
        const int RANDOM_SEED = 1013904223L;    //((int)idLib::sys->GetClockTicks())

        SIMDProcessor p_simd;
        SIMDProcessor p_generic;
        int baseClocks = 0;

        void StartRecordTime(start )
            => start = mach_absolute_time();

        void StopRecordTime(end )
            => end = mach_absolute_time();

        void GetBest(start, end, best )
        {
            if (!best || end - start < best)
                best = end - start;
        }

        void PrintClocks(string s, int dataCount, int clocks, int otherClocks = 0)
        {
            Lib.Printf(s);
            ...
            if (otherClocks && clocks)
            {
                ...
                var p = (int)((float)(otherClocks - clocks) * 100.0f / (float)otherClocks);
                Lib.Printf($"c = {dataCount:4}, clcks = {clocks:5}, {p:%}%\n", dataCount, clocks, p);
            }
            else
                Lib.Printf($"c = {dataCount:4}, clcks = {clocks:5}\n", , );
        }
        ...
        void TestMemcpy()
        {
            ...
            for (i = 5; i < 8192; i += 31)
            {
                ...
                for (j = 0; j < i; j++)
                {
                    if (test1[j] != test0[j])
                    {
                        Lib.Printf("   simd->Memcpy() " S_COLOR_RED "X\n");
                        return;
                    }
                }
            }
            Lib.Printf("   simd->Memcpy() ok\n");
        }
            ...
            for (i = 5; i < 8192; i += 31)
            {
                for (j = -1; j <= 1; j++)
                {
                    ...
                    for (k = 0; k < i; k++)
                    {
                        if (test[k] != (byte)j)
                        {
                            Lib.Printf("   simd->Memset() " S_COLOR_RED "X\n");
                            return;
                        }
                    }
                }
            }
            Lib.Printf("   simd->Memset() ok\n");
        }
        ...
        */
        void TestConvertJointMatsToJointQuats()
        {
            ...
            for (i = 0; i < COUNT; i++)
            {
                ...
                {
                    Lib.Printf("ConvertJointMatsToJointQuats: broken q %i\n", i);
                    break;
                }
                if (!joints1[i].t.Compare(joints2[i].t, 1e-4f))
                {
                    Lib.Printf("ConvertJointMatsToJointQuats: broken t %i\n", i);
                    break;
                }
            }
            result = (i >= COUNT) ? "ok" : S_COLOR_RED "X";
        ...
        */
        void TestDeriveTangents()
        {
            ...
            for (i = 0; i < COUNT; i++)
            {
                ...
                {
                    Lib.Printf("DeriveTangents: broken at normal %i\n -- expecting %s got %s", i, v1.ToString(), v2.ToString());
                    break;
                }
                ...
                if (!v1.Compare(v2, 1e-1f))
                {
                    Lib.Printf("DeriveTangents: broken at tangent0 %i -- expecting %s got %s\n", i, v1.ToString(), v2.ToString());
                    break;
                }
                ...
                if (!v1.Compare(v2, 1e-1f))
                {
                    Lib.Printf("DeriveTangents: broken at tangent1 %i -- expecting %s got %s\n", i, v1.ToString(), v2.ToString());
                    break;
                }
                if (!planes1[i].Compare(planes2[i], 1e-1f, 1e-1f))
                {
...
else
{
    ...
    Lib.SetRefreshOnPrint(true);

Lib.Printf("using %s for SIMD processing\n", p_simd->GetName());
...
public static void Test_f(CmdArgs args)
{
