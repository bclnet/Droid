...
namespace System.NumericsX.Core
{
    ...
    public unsafe class VFile : IDisposable
    {
        ...
        {
            common.FatalError("File::Read: cannot read from File");
            return 0;
        }
        public virtual int Read(byte[] buffer, int len)
        {
        ...
        {
            common.FatalError("File::Write: cannot write to File");
            return 0;
        }
        public virtual int Write(byte[] buffer, int len)
        {
    ...
    public class VFile_Memory : VFile
    {
        ...
        public override int Read(byte[] buffer, int len)
        {
            ...
            {
                common.FatalError($"File_Memory::Read: {name} not opened in read mode");
                return 0;
            }
        ...
        public override int Write(byte[] buffer, int len)
        {
            ...
            {
                common.FatalError($"File_Memory::Write: {name} not opened in write mode");
                return 0;
            }
            ...
            if (alloc > 0)
            {
                if (maxSize != 0)
                {
                    common.Error($"File_Memory::Write: exceeded maximum size {maxSize}");
                    return 0;
                }
    ...
    public class VFile_BitMsg : VFile
    {
        ...
        public override int Read(byte[] buffer, int len)
        {
            if ((mode & (1 << (int)FS.READ)) == 0)
            {
                common.FatalError($"File_BitMsg::Read: {name} not opened in read mode");
                return 0;
            }
    ...
    public class VFile_Permanent : VFile
    {
        ...
        public override int Read(byte[] buffer, int len)
        {
            ...
            if ((mode & (1 << (int)FS.READ)) == 0)
            {
                common.FatalError($"File_Permanent::Read: {name} not opened in read mode");
                return 0;
            }
            ...
            while (remaining != 0)
            {
                ...
                if (read == -1) common.FatalError($"File_Permanent::Read: -1 bytes read from {name}");
        ...
        public override int Write(byte[] buffer, int len)
        {
            ...
            if ((mode & (1 << (int)FS.WRITE)) == 0)
            {
                common.FatalError("File_Permanent::Write: {name} not opened in write mode");
                return 0;
            }
            ...
            while (remaining != 0)
            {
                ...
                if (written == 0)
                    if (tries == 0) tries = 1;
                    else { common.Printf($"File_Permanent::Write: 0 bytes written to {name}\n"); return 0; }
                if (written == -1) { common.Printf($"File_Permanent::Write: -1 bytes written to {name}\n"); return 0; }
        ...
        public override int Length => fileSize;
        public override DateTime Timestamp => SysW.FileTimeStamp(null);
    ...
    public class VFile_InZip : VFile
    {
        ...
        public override int Write(byte[] buffer, int len)
        {
            common.FatalError($"File_InZip::Write: cannot write to the zipped file {name}");
            return 0;
        }
        public override int Length => fileSize;
        public override DateTime Timestamp => DateTime.MinValue;
        public override int Tell => unztell(z);
        public override void ForceFlush() => common.FatalError($"File_InZip::ForceFlush: cannot flush the zipped file {name}");
        public override void Flush() => common.FatalError($"File_InZip::Flush: cannot flush the zipped file {name}");
        ...
        public override int Seek(long offset, FS_SEEK origin)
        {
            ...
            switch (origin)
            {
                ...
                default:
                    {
                        common.FatalError($"File_InZip::Seek: bad origin for {name}\n");
                        break;
                    }
            }
            return -1;
