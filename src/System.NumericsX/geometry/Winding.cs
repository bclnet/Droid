    ...
    public class Winding
    {
        ...
        public unsafe int Split(Plane plane, float epsilon, out Winding front, out Winding back)
        {
            ...
            if (counts[SIDE_FRONT] == 0 && counts[SIDE_BACK] == 0)
            {
                ...
                if (windingPlane.Normal * plane.Normal > 0f)
                {
                    front = Copy();
                    return SIDE_FRONT;
                }
        ...
        public unsafe Winding Clip(Plane plane, float epsilon = Plane.ON_EPSILON, bool keepOn = false)
        {
            ...
            var newPoints = stackalloc Vector5[maxpts];
            newNumPoints = 0;
        ...
        {
            var w = new Winding(numPoints);
            w.numPoints = numPoints;
            fixed (void* w_p = w.p, p = this.p)
                Unsafe.CopyBlock(w_p, p, (uint)(numPoints * sizeof(Vector5)));
            return w;
        }
        ...
        public void RemoveEqualPoints(float epsilon = Plane.ON_EPSILON)
        {
            ...
            for (i = 0; i < numPoints; i++)
            {
                if ((p[i].ToVec3() - p[(i + numPoints - 1) % numPoints].ToVec3()).LengthSqr >= MathX.Square(epsilon))
                    continue;
                numPoints--;
        ...
        public void RemoveColinearPoints(Vector3 normal, float epsilon = Plane.ON_EPSILON)
        {
            ...
            for (i = 0; i < numPoints; i++)
            {
                ...
                if (MathX.Fabs(edgeNormal * p[(i + 1) % numPoints].ToVec3() - dist) > epsilon)
                    continue;
        ...
        public bool InsertPointIfOnEdge(Vector3 point, Plane plane, float epsilon = Plane.ON_EPSILON)
        {
            ...
            if (MathX.Fabs(plane.Distance(point)) > epsilon)
                return false;
        ...
        public unsafe void AddToConvexHull(Winding winding, Vector3 normal, float epsilon = Plane.ON_EPSILON)
        {
            ...
            for (i = 0; i < winding.numPoints; i++)
            {
                ...
                fixed (Vector5* p = this.p)
                    Unsafe.CopyBlock(p, newHullPoints, (uint)(numNewHullPoints * sizeof(Vector5)));
            }
        }
        ...
        public unsafe void AddToConvexHull(Vector3 point, Vector3 normal, float epsilon = Plane.ON_EPSILON)
        {
            ...
            fixed (Vector5* p = this.p)
                Unsafe.CopyBlock(p, hullPoints, (uint)(numHullPoints * sizeof(Vector5)));
        }
        ...
        public bool LineIntersection(Plane windingPlane, Vector3 start, Vector3 end, bool backFaceCull = false)
        {
            ...
            // if back face culled
            if (backFaceCull && front < 0f)
                return false;

            // get point of intersection with winding plane
            Vector3 mid;
            if (MathX.Fabs(front - back) < 0.0001f)
                mid = end;
            else
            {
        ...
        protected unsafe virtual bool ReAllocate(int n, bool keep = false)
        {
            ...
            {
                if (keep)
                    fixed (Vector5* p = this.p, oldP_ = oldP)
                        Unsafe.CopyBlock(p, oldP_, (uint)(numPoints * sizeof(Vector5)));
            }
            allocedSize = n;
        ...
        public unsafe int Split(FixedWinding back, Plane plane, float epsilon = Plane.ON_EPSILON)
        {
            ...
            fixed (Vector5* p = this.p)
            {
                for (i = 0; i < numPoints; i++)
                {
                    ...
                    if (!winding.EnsureAlloced(winding.numPoints + 1, true))
