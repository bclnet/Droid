...
class DroidNet {
    ...
    fun mono(context: Context, domain: String, name: String) {
        ...
        for (file in LibFiles)
            AssetHelper.copyAsset(assets, "$dataDir/lib", file, "lib/$file")
        for (file in LibArchFiles)
            AssetHelper.copyAsset(assets, "$dataDir/lib", file, "lib-$arch/$file")
        for (file in EtcFiles)
            AssetHelper.copyAsset(assets, "$dataDir/etc", file, "etc/$file")
        for (file in domainFiles)
            AssetHelper.copyAsset(assets, "$dataDir/lib", file, "domain/$file")
        //AssetHelper.printFolder(dataDir, true)
