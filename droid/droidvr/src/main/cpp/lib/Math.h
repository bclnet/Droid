...
#define Math_Between(min, a, max)		((min)<(a)&&(a)<(max))
//#define Math_Clamp(min, a, max)		(Math_ClampMin(Math_ClampMax(a, max), min))
#define Math_Clamp(min, a, max)			((a)>=(min)?((a)<(max)?(a):(max)):(min))
#define Math_ClampMax(a, max)			((a)<(max)?(a):(max))
#define Math_ClampMin(a, min)			((a)>=(min)?(a):(min))
...
#define Math_Len(a, b)					sqrtf(powf(a, 2.0f) + powf(b, 2.0f))
...
#define Math_Vector2Norm(a, _)			{ float l=sqrtf(Math_DotProduct(a,a)); if(l)l=1.0f/l; _[0]=a[0]*l; _[1]=a[1]*l; _[2]=a[2]*l; }
...
#define Math_Vector3Avg2(a, b, _)		((_)[0]=((a)[0]+(b)[0])*0.5, (_)[1]=((a)[1]+(b)[1])*0.5, (_)[2]=((a)[2]+(b)[2])*0.5)
...
#define Math_Vector3Len(a)				sqrtf(Math_DotProduct(a,a))
...
#define Math_Vector3Norm(_)				{ float l=sqrtf(Math_DotProduct(_,_)); if(l)l=1.0f/l; _[0]*=l; _[1]*=l; _[2]*= l; }
#define Math_Vector3NormFast(_)			{ float l=Math::Rsqrt(Math_DotProduct(_,_)); _[0]*=l; _[1]*=l; _[2]*=l; }
#define Math_Vector3NormLen(_)			Math::Vector2NormLen((_),(_))
