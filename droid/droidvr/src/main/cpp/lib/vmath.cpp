...
void M::RotateAboutOrigin(float x, float y, float rotation, vec2_t _) {
    _[0] = cosf(M_DEG2RAD(-rotation)) * x + sinf(M_DEG2RAD(-rotation)) * y;
    _[1] = cosf(M_DEG2RAD(-rotation)) * y - sinf(M_DEG2RAD(-rotation)) * x;
}

float M::NonLinearFilter(float v) {
    float val = 0.0f;
    if (v > NLF_DEADZONE) {
        val = v;
        val -= NLF_DEADZONE;
...
double M::ClockTimeInMilliSeconds() {
    struct timespec now;
    clock_gettime(CLOCK_MONOTONIC, &now);
    return (now.tv_sec * 1e9 + now.tv_nsec) * (double) (1e-6);
}

void M::AnglesFromVectors(const vec3_t forward, const vec3_t right, const vec3_t up, vec3_t _) {
    float yaw, roll;
    float pitch = -asin(forward[2]);
    float cpitch = cos(pitch);
    if (fabs(cpitch) > EQUAL_EPSILON) {
        cpitch = 1.0f / cpitch;
        pitch = M_RAD2DEG(pitch);
        yaw = M_RAD2DEG(atan2(forward[1] * cpitch, forward[0] * cpitch));
        roll = M_RAD2DEG(atan2(-right[2] * cpitch, up[2] * cpitch));
    } else {
        pitch = forward[2] > 0 ? -90.0f : 90.0f;
        yaw = M_RAD2DEG(atan2(right[0], -right[1]));
        roll = 180.0f;
    }
    _[M_PITCH] = pitch;
    _[M_YAW] = yaw;
...
void M::AnglesFromVectors2(const vec3_t forward, const vec3_t right, const vec3_t up, vec3_t _) {
    ...
    cr = cos(roll);
    sr = sin(roll);
    if (fabs(cy) > EQUAL_EPSILON) cp = cp_x_cy / cy;
    else if (fabs(sy) > EQUAL_EPSILON) cp = cp_x_sy / sy;
    else if (fabs(sr) > EQUAL_EPSILON) cp = cp_x_sr / sr;
    else if (fabs(cr) > EQUAL_EPSILON) cp = cp_x_cr / cr;
    else cp = cos(asin(sp));
    float pitch = atan2(sp, cp);
    _[0] = pitch / (M_PI * 2.f / 360.f);
    _[1] = yaw / (M_PI * 2.f / 360.f);
    _[2] = roll / (M_PI * 2.f / 360.f);
    AnglesNorm(_);
}

void M::AnglesInterpolate(vec3_t start, vec3_t end, vec3_t _, float frac) {
    float d, ang1, ang2;
    for (int i = 0; i < 3; i++) {
        ang1 = start[i];
        ang2 = end[i];
...
void M::AnglesNorm(vec3_t angles) {
...
void M::AnglesToVectors(const vec3_t angles, vec3_t forward, vec3_t right, vec3_t up) {
    ...
    SinCosFastVector3(Math_DEG2RAD(angles[M_YAW]), Math_DEG2RAD(angles[M_PITCH]), Math_DEG2RAD(angles[M_ROLL]),
        ...
        &cy, &cp, &cr);
#else
    SinCos(M_DEG2RAD(angles[M_YAW]), &sy, &cy);
    SinCos(M_DEG2RAD(angles[M_PITCH]), &sp, &cp);
    SinCos(M_DEG2RAD(angles[M_ROLL]), &sr, &cr);
#endif
    if (forward) {
        forward[0] = cp * cy;
        forward[1] = cp * sy;
...
void M::AnglesQuaternion(const vec3_t angles, vec4_t _) {
    float sr, sp, sy, cr, cp, cy;
#ifdef XASH_VECTORIZE_SINCOS
...
float M::ApproachValue(float target, float value, float speed) {
...
bool M::BoundsAndSphereIntersect(const vec3_t mins, const vec3_t maxs, const vec3_t origin, float radius) {
...
bool M::BoundsIntersect(const vec3_t mins1, const vec3_t maxs1, const vec3_t mins2, const vec3_t maxs2) {
...
unsigned short M::FloatToHalf(float value) {
...
float M::HalfToFloat(unsigned short value) {
...
void M::Matrix3x4_ConcatTransforms(matrix3x4 _, cmatrix3x4 m1, cmatrix3x4 m2) {
...
void M::Matrix3x4_CreateFromEntity(matrix3x4 _, const vec3_t angles, const vec3_t origin, float scale) {
...
void M::Matrix3x4_FromOriginQuat(matrix3x4 _, const vec4_t quaternion, const vec3_t origin) {
...
const matrix3x4 M::Matrix3x4_Identity = {
        {1, 0, 0, 0},    // PITCH [forward], org[0]
        {0, 1, 0, 0},    // YAW [right]    , org[1]
        {0, 0, 1, 0},    // ROLL [up]      , org[2]
};

void M::Matrix3x4_InvertSimple(matrix3x4 _, cmatrix3x4 m) {
...
void M::Matrix3x4_OriginFromMatrix(cmatrix3x4 m, float *_) {
    _[0] = m[0][3];
    _[1] = m[1][3];
    _[2] = m[2][3];
}

void M::Matrix3x4_SetOrigin(matrix3x4 _, float x, float y, float z) {
    _[0][3] = x;
    _[1][3] = y;
    _[2][3] = z;
}

void M::Matrix3x4_TransformPositivePlane(cmatrix3x4 m, const vec3_t normal, float d, vec3_t _, float *dist) {
...
void M::Matrix3x4_VectorIRotate(cmatrix3x4 m, const float v[3], float _[3]) {
    _[0] = v[0] * m[0][0] + v[1] * m[1][0] + v[2] * m[2][0];
    _[1] = v[0] * m[0][1] + v[1] * m[1][1] + v[2] * m[2][1];
    _[2] = v[0] * m[0][2] + v[1] * m[1][2] + v[2] * m[2][2];
}

void M::Matrix3x4_VectorITransform(cmatrix3x4 m, const float v[3], float _[3]) {
    vec3_t dir;
    dir[0] = v[0] - m[0][3];
    dir[1] = v[1] - m[1][3];
    dir[2] = v[2] - m[2][3];
...
void M::Matrix4x4_ConcatTransforms(matrix4x4 _, cmatrix4x4 m1, cmatrix4x4 m2) {
    _[0][0] = m1[0][0] * m2[0][0] + m1[0][1] * m2[1][0] + m1[0][2] * m2[2][0];
    _[0][1] = m1[0][0] * m2[0][1] + m1[0][1] * m2[1][1] + m1[0][2] * m2[2][1];
    _[0][2] = m1[0][0] * m2[0][2] + m1[0][1] * m2[1][2] + m1[0][2] * m2[2][2];
    _[0][3] = m1[0][0] * m2[0][3] + m1[0][1] * m2[1][3] + m1[0][2] * m2[2][3] + m1[0][3];
...
void M::Matrix4x4_ConvertToEntity(cmatrix4x4 m, vec3_t angles, vec3_t origin) {
    float xyDist = sqrt(m[0][0] * m[0][0] + m[1][0] * m[1][0]);
    // enough here to get angles?
    if (xyDist > 0.001f) {
        angles[0] = M_RAD2DEG(atan2(-m[2][0], xyDist));
        angles[1] = M_RAD2DEG(atan2(m[1][0], m[0][0]));
        angles[2] = M_RAD2DEG(atan2(m[2][1], m[2][2]));
    }
        // forward is mostly Z, gimbal lock
    else {
        angles[0] = M_RAD2DEG(atan2(-m[2][0], xyDist));
        angles[1] = M_RAD2DEG(atan2(-m[0][1], m[1][1]));
        angles[2] = 0.0f;
    }
    origin[0] = m[0][3];
    origin[1] = m[1][3];
...
void M::Matrix4x4_CreateFromEntity(matrix4x4 _, const vec3_t angles, const vec3_t origin, float scale) {
    float angle, sr, sp, sy, cr, cp, cy;
    if (angles[M_ROLL]) {
#ifdef XASH_VECTORIZE_SINCOS
        SinCosFastVector3(Math_DEG2RAD(angles[M_YAW]), Math_DEG2RAD(angles[M_PITCH]), Math_DEG2RAD(angles[M_ROLL]),
...
void M::Matrix4x4_CreateTranslate(matrix4x4 _, double x, double y, double z) {
    _[0][0] = 1.0f;
    _[0][1] = 0.0f;
    _[0][2] = 0.0f;
    _[0][3] = x;
...
void M::Matrix4x4_FromOriginQuat(matrix4x4 _, const vec4_t quaternion, const vec3_t origin) {
    _[0][0] = 1.0f - 2.0f * quaternion[1] * quaternion[1] - 2.0f * quaternion[2] * quaternion[2];
    _[1][0] = 2.0f * quaternion[0] * quaternion[1] + 2.0f * quaternion[3] * quaternion[2];
    _[2][0] = 2.0f * quaternion[0] * quaternion[2] - 2.0f * quaternion[3] * quaternion[1];
    _[0][3] = origin[0];
...
const matrix4x4 M::Matrix4x4_Identity = {
        {1, 0, 0, 0},    // PITCH
        {0, 1, 0, 0},    // YAW
        {0, 0, 1, 0},    // ROLL
        {0, 0, 0, 1},    // ORIGIN
...
bool M::Matrix4x4_InvertFull(matrix4x4 _, cmatrix4x4 m) {
    float *temp;
    float *r[4];
    float rtemp[4][8];
    float m_[4];
...
void M::Matrix4x4_InvertSimple(matrix4x4 _, cmatrix4x4 m) {
...
void M::Matrix4x4_OriginFromMatrix(cmatrix4x4 m, float *_) {
    _[0] = m[0][3];
    _[1] = m[1][3];
    _[2] = m[2][3];
}

void M::Matrix4x4_SetOrigin(matrix4x4 _, float x, float y, float z) {
    _[0][3] = x;
    _[1][3] = y;
    _[2][3] = z;
}

void M::Matrix4x4_TransformPositivePlane(cmatrix4x4 m, const vec3_t normal, float d, vec3_t _, float *dist) {
    float scale = sqrt(m[0][0] * m[0][0] + m[0][1] * m[0][1] + m[0][2] * m[0][2]);
    float iscale = 1.0f / scale;
    _[0] = (normal[0] * m[0][0] + normal[1] * m[0][1] + normal[2] * m[0][2]) * iscale;
    _[1] = (normal[0] * m[1][0] + normal[1] * m[1][1] + normal[2] * m[1][2]) * iscale;
...
void M::Matrix4x4_TransformStandardPlane(cmatrix4x4 m, const vec3_t normal, float d, vec3_t _, float *dist) {
    float scale = sqrt(m[0][0] * m[0][0] + m[0][1] * m[0][1] + m[0][2] * m[0][2]);
    float iscale = 1.0f / scale;
    _[0] = (normal[0] * m[0][0] + normal[1] * m[0][1] + normal[2] * m[0][2]) * iscale;
    _[1] = (normal[0] * m[1][0] + normal[1] * m[1][1] + normal[2] * m[1][2]) * iscale;
...
void M::Matrix4x4_Transpose(matrix4x4 _, cmatrix4x4 m) {
    _[0][0] = m[0][0];
    _[0][1] = m[1][0];
    _[0][2] = m[2][0];
    _[0][3] = m[3][0];
...
void M::Matrix4x4_VectorIRotate(cmatrix4x4 m, const float v[3], float _[3]) {
    _[0] = v[0] * m[0][0] + v[1] * m[1][0] + v[2] * m[2][0];
    _[1] = v[0] * m[0][1] + v[1] * m[1][1] + v[2] * m[2][1];
    _[2] = v[0] * m[0][2] + v[1] * m[1][2] + v[2] * m[2][2];
}

void M::Matrix4x4_VectorITransform(cmatrix4x4 m, const float v[3], float _[3]) {
    vec3_t dir;
    dir[0] = v[0] - m[0][3];
    dir[1] = v[1] - m[1][3];
    dir[2] = v[2] - m[2][3];
...
void M::Matrix4x4_VectorRotate(cmatrix4x4 m, const float v[3], float _[3]) {
    _[0] = v[0] * m[0][0] + v[1] * m[0][1] + v[2] * m[0][2];
    _[1] = v[0] * m[1][0] + v[1] * m[1][1] + v[2] * m[1][2];
    _[2] = v[0] * m[2][0] + v[1] * m[2][1] + v[2] * m[2][2];
}

void M::Matrix4x4_VectorTransform(cmatrix4x4 m, const float v[3], float _[3]) {
    _[0] = v[0] * m[0][0] + v[1] * m[0][1] + v[2] * m[0][2] + m[0][3];
    _[1] = v[0] * m[1][0] + v[1] * m[1][1] + v[2] * m[1][2] + m[1][3];
    _[2] = v[0] * m[2][0] + v[1] * m[2][1] + v[2] * m[2][2] + m[2][3];
}

int M::NearestPow(int value, bool roundDown) {
    int n = 1;
    if (value <= 0) return 1;
    while (n < value) n <<= 1;
...
void M::QuaternionSlerp(const vec4_t p, vec4_t q, float t, vec4_t _) {
...
float M::RangeRemapValue(float value, float a, float b, float c, float d) { return c + (d - c) * (value - a) / (b - a); }

float M::Rsqrt(float value) {
    if (value == 0.0f)
        return 0.0f;
    float x = value * 0.5f;
    int i = *(int *) &value;
...
void M::SinCos(float radians, float *sin, float *cos) {
#if _MSC_VER == 1200
    _asm {
...
#ifdef XASH_VECTORIZE_SINCOS
void M::SinCosFastVector2(float r1, float r2,
    float* s0, float* s1,
    float* c0, float* c1) {
...
void M::SinCosFastVector3(float r1, float r2, float r3,
...
void M::SinCosFastVector4(float r1, float r2, float r3, float r4,
...
void M::SinFastVector3(float r1, float r2, float r3,
    float* s0, float* s1, float* s2) {
    v4sf rad_vector = { r1, r2, r3, 0 };
    v4sf sin_vector;
...
float M::Vector2NormLen(const vec3_t v, vec3_t _) {
...
void M::Vector3Angles(const float *forward, float *angles) {
    float tmp, yaw, pitch;
    if (!forward || !angles) {
        if (angles) M_Vector3Zero(angles);
        return;
    }
    ...
    M_Vector3Set(angles, pitch, yaw, 0);
}

void M::Vectors(const vec3_t forward, vec3_t right, vec3_t up) {
    right[0] = forward[2];
    right[1] = -forward[0];
    right[2] = forward[1];
    float d = M_DotProduct(forward, right);
    M_Vector3MA(right, -d, forward, right);
    M_Vector3Norm(right);
