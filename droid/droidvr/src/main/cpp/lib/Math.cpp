...
void Math::RotateAboutOrigin(float x, float y, float rotation, vec2_t _) {
	_[0] = cosf(Math_DEG2RAD(-rotation)) * x + sinf(Math_DEG2RAD(-rotation)) * y;
	_[1] = cosf(Math_DEG2RAD(-rotation)) * y - sinf(Math_DEG2RAD(-rotation)) * x;
}

float Math::NonLinearFilter(float v) {
	float val = 0.0f;
	if (v > NLF_DEADZONE) {
		val = v;
		val -= NLF_DEADZONE;
...
double Math::ClockTimeInMilliSeconds() {
	struct timespec now;
	clock_gettime(CLOCK_MONOTONIC, &now);
	return (now.tv_sec * 1e9 + now.tv_nsec) * (double)(1e-6);
}

void Math::AnglesFromVectors(const vec3_t forward, const vec3_t right, const vec3_t up, vec3_t _) {
	float yaw, roll;
...
void Math::AnglesFromVectors2(const vec3_t forward, const vec3_t right, const vec3_t up, vec3_t _) {
...
void Math::AnglesNorm(vec3_t angles) {
...
void Math::AnglesToVectors(const vec3_t angles, vec3_t forward, vec3_t right, vec3_t up) {
...
void Math::Vectors(const vec3_t forward, vec3_t right, vec3_t up) {
	right[0] = forward[2];
	right[1] = -forward[0];
	right[2] = forward[1];
